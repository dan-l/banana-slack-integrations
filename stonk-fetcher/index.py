...
def process(slack_request):
    ...
    text = parse(get_ticker_symbol(param_text), get_query_type(param_text))
    print(text)
...
def get_quote(ticker_symbol):
    ...
    result = '%s (%s)' % (ticker_name, ticker_symbol.upper())
    result += '\n'
